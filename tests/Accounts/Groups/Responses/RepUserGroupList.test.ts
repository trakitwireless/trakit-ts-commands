...
import { RepUserGroupList } from "../../../../src/Accounts/Groups/Responses/RepUserGroupList";
import { UserGroup } from "@trakit/objects";

describe("RepUserGroupList", () => {
	test("constructor creates instance with userGroups", () => {
		...
		const reply = new RepUserGroupList({ userGroups: [userGroup1, userGroup2] });
	...
	test("constructor creates instance with JSON", () => {
		const reply = new RepUserGroupList({
			userGroups: [
	...
	test("toJSON returns correct structure", () => {
		...
		const reply = new RepUserGroupList({ userGroups: [userGroup] });
