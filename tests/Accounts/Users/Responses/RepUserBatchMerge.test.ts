...
describe("RepUserBatchMerge", () => {
	test("constructor creates instance with users", () => {
		...
		const reply = new RepUserBatchMerge({ users: [user1, user2] });
	...
	test("constructor creates instance with JSON", () => {
		const reply = new RepUserBatchMerge({
			users: [
	...
	test("toJSON returns correct structure", () => {
		...
		const reply = new RepUserBatchMerge({ users: [user] });
