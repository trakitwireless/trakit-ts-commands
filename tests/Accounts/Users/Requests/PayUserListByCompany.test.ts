...
describe("PayUserListByCompany", () => {
	test("constructor creates instance with company", () => {
		const payload = new PayUserListByCompany({ company: { id: 123n } });
	...
	test("constructor creates instance with includeDeleted", () => {
		const payload = new PayUserListByCompany({ company: { id: 456n }, includeDeleted: true });
		expect(payload.company.id).toBe(456n);
		expect(payload.includeDeleted).toBe(true);
	});

	test("constructor creates instance without parameters", () => {
		const payload = new PayUserListByCompany();
		expect(payload.company.id).toBeUndefined();
	});

	test("createReply returns RepUserListByCompany", () => {
		const payload = new PayUserListByCompany({ company: { id: 123n } });
		const reply = payload.createReply({});
		expect(reply).toBeInstanceOf(RepUserListByCompany);
	});

	test("toJSON returns correct structure", () => {
		const payload = new PayUserListByCompany({ company: { id: 789n }, includeDeleted: true });
	...
	test("getCompanyId returns company id", () => {
		const payload = new PayUserListByCompany({ company: { id: 999n } });
