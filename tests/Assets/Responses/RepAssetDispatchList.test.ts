...
import { RepAssetDispatchListByCompany } from "../../../commands/Assets/Responses/RepAssetDispatchList";

describe('RepAssetDispatchListByCompany', () => {
	it('should create instance with assets data', () => {
		...
		const reply = new RepAssetDispatchListByCompany(json);
		expect(reply).toBeInstanceOf(RepAssetDispatchListByCompany);
		expect(reply).toBeInstanceOf(Reply);
		expect(reply.assets).toHaveLength(2);
		expect(reply.assets[0]).toBeInstanceOf(AssetDispatch);
